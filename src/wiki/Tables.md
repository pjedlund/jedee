...
- The header row is tinted with `--color-bg-accent` and set in capitals letterspaced by `--tracking-wide`, at `--size-step-min-2`, like the site's other capital labels. The caption is italic at `--size-step-min-1`. Body cells keep the size of the text around the table: Rutter warns against shrinking table text to fit more in, and a step smaller was tried and reverted. Cells have `--space-s` above and below.
