...
- **Places** (the activity index) — a slotted `[data-place-list]`: since 2026-09-15 the activities table, one `<tr data-lat data-lon>` per located activity, each dot named and linked from the row's first link ([[Tables]]). The table *is* the data source and the no-JS / screen-reader path: Leaflet's own markers have broken keyboard and SR handling upstream, so nobody is forced through the map to reach a post. JavaScript reads the rows that carry coordinates and drops one dot each into the box above; rows without coordinates stay in the table, unmapped. Each row's `data-activity` sets a `--place-color` in `place-map.css`, read once per row for its dot: orienteering orange, hikes `green-vivid`, runs `blue-vivid`.
