...
- ⚠ **A picker lists drafts unless told not to.** Picking one gives an empty section on the live page, because the build leaves it out. The `/now` pickers carry `filters: [{ field: draft, values: [true], exclude: true }]`. It has to be written as "exclude true" and not "only false", because many published posts have no `draft` line at all, and `values: [false]` would hide them (`fields/relation/helpers/filters.js`, checked in 0.209.1).
