...
⚠ **A hand-written utility that shares a name with a Tailwind core utility loses to it, and that measurement is no longer a guarantee.** On 2026-09-10 the new `utilities/numerals.css` defined `.tabular-nums`, `.lining-nums` and `.slashed-zero`. All three are also Tailwind `fontVariantNumeric` classes, and the content scan generated Tailwind's versions from the markup that used them. `tailwindUtilities` is the last layer, so its `font-variant-numeric: var(--tw-ordinal) var(--tw-slashed-zero) …` replaced the hand-written value. `.fraction`, which is not a Tailwind name, was the only one of the four that worked. The fix was switching the plugin off in `corePlugins`, which was safe because no markup had ever used Tailwind's versions. Before naming a new utility, check it against Tailwind's class list. See [[Typographic conventions]].
