...
EE's own `custom-youtube.webc` puts its styles in a top-level `<style>` block, and that block ships **inline in the body of every page that embeds a video, un-minified, comments and all.** It never reaches the site's CSS pipeline, so it is not deduplicated across embeds and not compressed. Upstream that block is 730 bytes; jedee's is 3,020, grown as the poster fade, the themed placeholder and the focus-ring fix were added to it. On a jam page the body carries 3.5 kB of inline `<style>` against a 50 kB page — about 7%, for CSS that is identical on every such page.
