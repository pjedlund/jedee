...
Photo and jam were widened on 2026-09-09: a shared photo previews as the photograph, a shared jam as a card built around its album cover. That is 122 pages, and the head now picks from three sources, most specific first — a jam's composited card, then the post's own artwork, then Eleventy Excellent's generated title card, then the site default.
...
For **jam** the bare cover was correct but square, and a square is a small tile in a feed built for a wide card. `src/_config/setup/generate-og-jams.js` (`npm run og:jams`) draws the composited version instead — art on the left, an orange "Listening to" eyebrow, title and artist on the right, the signature mark bottom-right. **This one does commit a file per post**: 120 of them, ~11 MB, generated on the laptop and carrying every cost in the ⚠ list. That was a deliberate trade for how much better a wide card reads, made after looking at both side by side, not a default.
...
⚠ **The jam cards are generated but `clean:og` does not regenerate them.** `npm run clean:og` rimrafs all of `src/assets/og-images/`, which now includes `jams/` — and the `npm start` that follows only rebuilds the *article* SVG cards. After a `clean:og`, run `npm run og:jams` too or every jam silently drops back to its square cover.

⚠ **A jam card goes stale in silence.** Nothing re-runs the generator, so changing a cover URL or a title leaves the old card in place — and a *retitled* jam is worse, because the slug moves and `ogCard` stops finding the file, dropping that jam to the square cover with no error. Re-run `npm run og:jams` after either.
...
**A jam's `og:description` was the other half of the fix.** It fell through to the site-wide "Personal site of Johan Edlund" — true, and useless beside a picture of a record. Only 6 of 120 jams carry a `description:` of their own, so `autoDescription` (in `src/_config/utils/jam-og-card.js`) composes the line the card itself cannot show, from fields the posts already have: artist 120/120, genre 113, year 99, album 101. `Solace` becomes "Jakob (2006) · rock, post-rock, instrumental". It sits *below* an authored `description:` in the head's chain, so it never overrides one, and the album is dropped when it merely repeats the title. Any type can opt in the same way by computing an `autoDescription`.

**Coverage after this**: 120 jams and 2 photos carry their own card, `/jams/genres/` correctly still takes the default, and no jam falls through — all 120 have a `cover:`. The ranked list of what is left, in the order the artwork makes easy: notes (22), then watching (10) and reading (8) on their `cover:`, then video (3) on the self-hosted YouTube poster, and recipe (1). Activities (180) and the five response types (60) are deliberately left on the default — nobody shares an activity, and for a response the platform's preview of the *target* page is the more useful card. **Notes need the generated card, not artwork**, and are straightforward: every post of every type carries a `title:` in front matter, all sixteen types and 416 posts, so a note card is the article card with a second pagination source.
