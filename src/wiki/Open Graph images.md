...
Photo and jam were widened on 2026-09-09 without generating anything: a shared photo previews as the photograph, a shared jam as its album cover. That is 122 pages, and the reason to prefer it wherever a type already carries an image is everything in the ⚠ list above — no committed JPEG, nothing to strand on a retitle, no `existsSync` guard to clear, no font dependency. A generated title card is what you build for a type that has *only* words.
