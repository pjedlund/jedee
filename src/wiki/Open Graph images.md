...
⚠ **Nothing ever deletes an image.** The `existsSync` guard only ever adds. `src/assets/og-images/` currently holds 17 JPEGs against 13 article files, and seven of them — `demo-pages`, `post-with-301-redirects`, `post-with-a-gallery`, `post-with-a-video`, `post-with-all-the-markdown`, `post-with-fetched-content`, `post-with-some-code` — belong to Eleventy Excellent demo articles that were deleted from the repo. They are committed, they ship in the build, and nothing references them.
