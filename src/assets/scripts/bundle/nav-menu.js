// POST-TYPE MEGA-MENU — a single disclosure that drops the post-type panel down from the header. Progressive enhancement: without this script no button is injected and the panel is a plain, visible list of links. © pattern: Manuel Matuzović https://web.dev/website-navigation/ + Web Accessibility Cookbook.

// Target the main nav by id, not the first <nav> on the page: the breadcrumb (partials/breadcrumb.njk) is a <nav> that precedes this one in the DOM.
...
// The critical CSS (nav-menu-cls.css) sets display:none on the panel while JS is enabled, so it can't flash open before this runs. Hand layout back to CSS (grid); visibility — driven by aria-expanded — is what now opens and closes it.
