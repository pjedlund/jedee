// Adapter in front of the existing Micropub endpoint: Health Auto Export's REST-API automations POST a workout here; this file filters it, translates it into the same flat properties a Micropub client would send, and hands it to the existing endpoint in-process (see the handler at the bottom — same auth check, same commit path as every other client, just a function call instead of an HTTP round-trip).
//
// Two automations point at this one file: a timer-based one for the routine Running/Gym net, and a manual-trigger-only one (Health Auto Export's own "Manual Export" action) with `?mode=manual` on the URL, which skips the activity-type filter entirely — for one-off activities outside that net (a hike, a ski tour).
//
...
// ponytail: seeded from Apple's known HealthKit workout-type display names. Confirm against a real Health Auto Export payload during setup (see the design doc) before trusting this list long-term — Apple's exact strings for Johan's watch aren't verified yet.
...
// Health Auto Export's documented workout start/end format: "yyyy-MM-dd HH:mm:ss Z" (e.g. "2026-06-29 08:30:00 +0200") — not spec-compliant ISO 8601 (missing the `T` separator and the offset's colon), so plain `new Date()` parsing of it is unreliable across engines. Reassemble it into a real ISO string instead of trusting Date parsing.
...
// Health Auto Export reports distance as { qty, units }; the site's `distanceKm` frontmatter has no unit field, so this adapter is the only place a unit conversion can happen. Rounded to 2 decimals (matches how distanceKm already reads on the backfilled posts).
...
// Health Auto Export sends Apple's workout *display name* ("Running", "Trail Running"); the site stores — and paceOrSpeed (src/_config/filters/pace.js) recognizes — its own short lowercase vocabulary ("run", "trailrun"). Without this a plain run would store activityType:"Running", which pace.js's FOOT set doesn't know, so the derived pace line would silently vanish. Map the paced foot activities whose site token is known; anything else (strength, or a one-off manual export) passes through unchanged for Johan to relabel on review. Keyed by the same space-stripped/lowercased form pace.js itself normalizes to.
...
// Same folder+slug logic the real post will get (workoutFile, already exported from micropub.js) — lets the handler check for a collision BEFORE forwarding, using a throwaway source path (only its directory portion survives workoutFile's rewrite).
...
// Translate one Health Auto Export workout into the flat Micropub properties the existing WORKOUT_KEY map already reads, plus everything the handler needs to post and dedupe it. Pure — no I/O. Returns null when required fields are missing.
